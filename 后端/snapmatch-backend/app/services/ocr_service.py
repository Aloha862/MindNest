...
def _normalize_any_block(item: Any, default_source: str = "tongyi") -> dict | None:
    ...
    if not isinstance(item, dict):
        return None
    text = _pick_text_from_dict(item)
    if not text:
        return None
    return {
        "text": text,
...
def _normalize_vlm_ocr_result(result: Any) -> dict:
    ...
    return {"rawText": str(raw_text), "blocks": blocks, "formulaBlocks": formula_blocks}


def _run_vlm_ocr(image_path: str) -> dict:
    prompt = """
你是 SnapMatch 的 OCR 与公式识别模型。请准确识别图片中的中文、英文、数字、题号、选项、解析文字和数学公式。
...
"""
    prompt = """
你是 SnapMatch 的高精度 OCR 模型。请准确识别图片中的中文、英文、数字、题号、选项、解析文字和数学公式。
要求：
...
"""
    prompt = """
你是 SnapMatch 的 OCR 文本识别模型。请只做图片文字转写，不要做题目解析，不要重写答案，不要把公式重新排版成单独的公式块。
