...
def _register_font() -> str:
    ...
    for path in candidates:
        if path.exists():
            pdfmetrics.registerFont(TTFont("SnapMatchCN", str(path)))
            return "SnapMatchCN"
    return "Helvetica"
...
def build_export(db: Session, current_user: User, payload: ExportRequest) -> tuple[bytes, str, str]:
    ...
    if fmt in {"word", "docx"}:
        return _word(markdown), "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "snapmatch-export.docx"
    if fmt == "pdf":
        return _pdf(markdown), "application/pdf", "snapmatch-export.pdf"
    return markdown.encode("utf-8"), "text/markdown; charset=utf-8", "snapmatch-export.md"
