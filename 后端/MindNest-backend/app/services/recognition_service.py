...
def _execute_recognition_pipeline(db: Session, task: RecognitionTask, payload: RecognitionStartRequest, *, raise_errors: bool = True) -> dict | None:
    ...
    try:
        ...
        if payload.enableOCR:
            def ocr_stage():
                start = perf_counter()
                ocr_provider = settings.ocr_provider.lower().strip()
                ocr_image_path = file.original_path if ocr_provider in {"qwen", "dashscope", "aliyun", "tongyi", "vlm"} else image_path
                result = run_ocr(ocr_image_path)
                cost = int((perf_counter() - start) * 1000)
